...
export default {
    // La funcion allMovies contendra el getAll de genericMongoCrud y le pasamos como parametros dos argumentos: req (la solicitud HTTP) y res (la respuesta HTTP).
    allMovies: async (req, res) => {
        try {
            const result = await genericMongoCrud.getAll(process.env.COLL_MOVIES)
            res.json(result)
        } catch (error) {
